---
title: "Visualizing genomic coverage data with ggplot"
...
Genomic coverage plots are a great and readily interpretable way to visualizing epigenetic profiles across many samples mapped to the same reference. See some examples [here](https://www.nature.com/articles/s41588-021-00914-y/figures/4 "Bowfin genome ATAC"), [here](https://genome.cshlp.org/content/32/6/1058/F3.expansion.html "Rattlesnake ChIP-ATAC"), and [here](https://academic.oup.com/view-large/figure/474290094/evae110f5.jpg "Rattlesnake ATAC"). The default method to visualize genomic data in this manner tends to be [IGV](https://igv.org/), which, while incredibly versatile often can be a challenge to wrangle and customize for publication-ready figures. Here is a tutorial on using ggplot and R to have much more artistic control over genomic coverage figures. While meant primarily for genomic data, some aspects of this tutorial can help you visualize any continuous data. The structure of intermediate objects will be shown so they can be easily replicated with custom data.

The R script used in this tutorial can be found here: [genome_coverage_minimal_example.R][1].

[1]:{{ site.url }}{{ site.baseurl }}/assets/blog_files/genome-coverage-vis/genome_coverage_minimal_example.R
...
If you want to do a bit more customization: [`scales`](https://scales.r-lib.org/), [`gggenes`](https://github.com/wilkox/gggenes/tree/master) and [`RColorBrewer`](https://cran.r-project.org/web/packages/RColorBrewer/index.html) are useful.

<br>

## Making coverage plots
`ggcoverage` is a package that has its own genomic coverage visualization implementation, but you'll get more flexibility with `ggplot`. It does however have a nice function to import many [commonly used coverage datatypes](https://rdrr.io/cran/ggcoverage/man/LoadTrackFile.html) (bam, wig, bw, bedgraph, txt) efficiently, without allowing massive files to overload your RAM. 

In this guided example, I'm using [data](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA1061517/ "Data") from a [recent publication](https://doi.org/10.1093/gbe/evae110 "GBE 2024") of ATAC-seq data mapped to a common reference genome, producing .bam alignment files. 

<br>

#### 1. Set up metadata, and load in coverage files 

The first step is to make a metadata object to let `ggcoverage` know where to look for data, and what it consists of, and any other useful data attributes.
...
|:----------------------------------------- |:--------:|:-------:|
| CV0857_viridis_North_M.dedup.filtered.unique | CV0857   | viridis |
| CV1081_viridis_Mid_M.dedup.filtered.unique   | CV1081   | viridis |
| CV1082_viridis_South_M.dedup.filtered.unique | CV1082   | viridis |
| CV1086_viridis_South_M.dedup.filtered.unique | CV1086   | viridis |
| CV1087_viridis_North_F.dedup.filtered.unique | CV1087   | viridis |
| CV1089_viridis_South_M.dedup.filtered.unique | CV1089   | viridis |
| CV1095_viridis_North_M.dedup.filtered.unique | CV1095   | viridis |
| CV0985_concolor_Other_F.dedup.filtered.unique | CV0985   | concolor|
| CV1090_cerberus_Other_M.dedup.filtered.unique | CV1090   | cerberus|
...
```
The resulting `track.df` object will provide scores (RPKM normalized by default, this can be changed) representing levels of coverage across genomic coordinates, either single nucleotides or larger bins. Note that the attributes we added to `sample.meta`, *SampleID* and *Species*, also appear in this object. This will allow us to group and colour our plot. You may notice that this dataframe is already in [`tidy`](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html) format, ideal for `ggplot`. See below for a snapshot of the dataframe: 
...
<br>
#### 3. Making the most basic coverage plot

First, I'll show you how to make them most basic plot, then increase it's complexity by building off the starter code.

The data used here represents ATAC-seq data from 3 species. For these plots, I want to split up coverage tracks to show each individual, and colour by species. This process involves a method called ["facet wrapping"](https://ggplot2.tidyverse.org/reference/facet_wrap.html). Here, we facet wrap by SampleID, then set `fill=Species`. We use `geom_col` because [we want heights to represent data values](https://ggplot2.tidyverse.org/reference/geom_bar.html). 
...
`ggplot` allows a great deal of customization and complexity. For example, we can add the following customizations:
- Single column facet
...
Adding annotations to ggplot is easy, it's just a matter of making new dataframes with information that you want to plot, then setting the appropriate x, y and potentially fill and colour values in the `aes()` call.

Lets add the following (note that this data is made up for the sake of illustration):
- A highlighted region representing a peak called by MACS2
...
<div style="text-align: center;">
  ...
  <figcaption>Even more customized plot, with three types of annotated data.</figcaption>
</div>

<br>

## Additional steps
`ggplot` offers far more customization potential than IGV, making it much more useful for making publication-ready genomic track figures. Touching up figures from `ggplot` can be done with other software such as Adobe Illustrator or InkScape for example.